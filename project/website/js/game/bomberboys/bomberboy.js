...
function requestBomb(id){
	...
	if(GAME.params[id+"_lastBomb"] >= BOMB_TIMEOUT && BOARD_MAT[startX][startY] != PLAYER_BOMB 
		...
		var properties = {};
		properties[""+id+"_bomb_xPos"] = starting_positions[count][0];
		properties[""+id+"_bomb_yPos"] = starting_positions[count][1];
	...
	for(var i = 0; i < GAME.playerIDs.length; i++){
		if(GAME.params[GAME.playerIDs[i]+"_alive"]){
			if(GAME.params[GAME.playerIDs[i]+"_xPos"] == x && GAME.params[GAME.playerIDs[i]+"_yPos"] == y){
				GAME.canvs["board"].dynamicObjects[""+GAME.playerIDs[i]].expired = true;
				BOARD_MAT[x][y] = DEATH;
